...
class Reserva:
    ...
    @staticmethod
    def crear(id_cliente, id_cabana, fecha_inicio, fecha_fin, precio_final, id_empleado):
        cursor, connection = create_connection()
        if cursor and connection:
            query = """
            INSERT INTO Reserva (id_cliente, id_cabana, fecha_inicio, fecha_fin, precio_final, id_empleado)
            VALUES (%s, %s, %s, %s, %s, %s)
            """
            cursor.execute(query, (id_cliente, id_cabana, fecha_inicio, fecha_fin, precio_final, id_empleado))
            connection.commit()
            id_reserva = cursor.lastrowid
            close_connection(connection, cursor)
            return Reserva(id_reserva, id_cliente, id_cabana, fecha_inicio, fecha_fin, precio_final, id_empleado)
        else:
    ...
    def actualizar(self, id_cliente=None, id_cabana=None, fecha_inicio=None, fecha_fin=None, precio_final=None, id_empleado=None):
        cursor, connection = create_connection()
        if cursor and connection:
            query = """
            UPDATE Reserva
            ...
            WHERE id_reserva = %s
            """
            cursor.execute(query, (id_cliente, id_cabana, fecha_inicio, fecha_fin, precio_final, id_empleado, self.id_reserva))
            connection.commit()
            close_connection(connection, cursor)
