...
class App:
    ...
    def mostrar_formulario_reserva(self):
        ...
        tk.Label(self.root, text="Actividad").pack()
        cabanas = Actividad.getAll()
        cabana_opciones = [f"{Actividad.id_actividad} - {actividad.nombre}" for actividad in actividad]
        id_cabana_combobox = ttk.Combobox(self.root, values=cabana_opciones)
        id_cabana_combobox.pack()
        ...
        # Función para actualizar el precio final basado en la cabaña seleccionada
        def actualizar_precio(event):
            seleccion = id_cabana_combobox.get()
            if seleccion:
                id_cabana = seleccion.split(" - ")[0]
                cabana = next((c for c in cabanas if c.id_cabana == id_cabana), None)
                if cabana:
                    precio_final_entry.delete(0, tk.END)
                    precio_final_entry.insert(0, cabana.costo)

        id_cabana_combobox.bind("<<ComboboxSelected>>", actualizar_precio)
        ...
        def on_submit():
            ...
            id_empleado = self.current_empleado.id_empleado if self.current_empleado else None
